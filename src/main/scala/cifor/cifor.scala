...
""" 
      ...
      val $bodyName: Int => Unit =  $body  // "if (InterruptObject.iflag) println(\"interrupt\");"
      var $index: Int = $init
...
v     */
  ...
  def ciforRangeMacro(c: Context)(r: c.Expr[Range])(body: c.Expr[Int => Unit]): c.Expr[Unit] = {
  ...
  def ciforRange2Macro(c: Context)(r1: c.Expr[Range], r2: c.Expr[Range])
    (body: c.Expr[(Int, Int) => Unit]): c.Expr[Unit] = {

    import c.universe._
    c.Expr[Unit](q"ciforRange($r1)(i => ciforRange($r2)(j => $body(i, j)))")
  }
}
...
object CiforSyntax {
  def cifor[A](init:A)(test:A => Boolean, next:A => A)(body:A => Unit): Unit =
    macro Syntax.ciforMacro[A]
  def cuforRange(r: Range)(body: Int => Unit): Unit =
    macro Syntax.ciforRangeMacro
  def cuforRange2(r1: Range, r2: Range)(body: (Int, Int) => Unit): Unit =
    macro Syntax.ciforRange2Macro
}
